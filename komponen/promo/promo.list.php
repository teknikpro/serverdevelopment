...
$mysql = "select id,ringkas,nama,create_date,alias,gambar,gambar1,startdate,enddate from tbl_$kanal where published='1' order by id  asc limit $ord, $judul_per_hlm";
$hasil = sql( $mysql);
...
while ($data =  sql_fetch_data($hasil)) {	
	...
	$datadetail[$id] = array("id"=>$id,"i"=>$i,"no"=>$no,"nama"=>$nama,"ringkas"=>$ringkas,"tanggal"=>$tanggal,"startdate"=>$startdate,"enddate"=>$enddate,"alias"=>$alias,"link"=>$link,"gambar"=>$gambar);
	$i++;
	$no++;
