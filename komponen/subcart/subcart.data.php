...
while ($row = sql_fetch_data($hsl))
{
	...
	$sql2	= "select title,secid,subid,jenisvideo,screenshot,harga from tbl_product_post where produkpostid='$produkpostid'";
	$query2	= sql($sql2);
	...
	$harga		= $row2['harga'];
	$link_produk	= "$fulldomain/produk/read/$aliasSecc/$aliasSubb/$produkpostid/$alias.html";
	...
	if(!empty($prodidup))
	{
		$prodidup1 = implode(",",$prodidup);
		$whereup = " and upsellingid NOT IN ( $prodidup1 )";
	}
	
	$dt_toko[$resellerid] = array("resellerid"=>$resellerid,"namatoko"=>$namatoko);

	$dt_keranjang[$resellerid][$id] = array("id"=>$id,"nama"=>$nama,"produkpostid"=>$produkpostid,"berat"=>$berat,"volume"=>$volume,"image_s"=>$image_s,"harga"=>$harga2,"qty"=>$qty,
