<?php
	$invoiceid = $var[4];
		...
		$sql	= "select transaksidetailid,produkpostid,jumlah,matauang,harga,totalharga,berat, resellerid from tbl_transaksi_detail 
		...
		while ($row = sql_fetch_data($hsl))
		{
			...
			$detailtoko[$resellerid] = array("resellerid"=>$resellerid,"namatoko"=>$namatoko,"ongkoskirim"=>$ongkoskirimpertoko);
						
			$detailproduk[$resellerid][$transaksidetailid] = array("id"=>$id,"xx"=>$xx,"namaprod"=>$namaprod,"harga"=>$harga,"total"=>$total,"qty"=>$jumlah,"kodeproduk"=>$kodeproduk);
			$xx++;
		...
		$tpl->assign("detailtoko",$detailtoko);
