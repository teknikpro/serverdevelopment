...
if ($subaksi == "finish") {
	...
	$voucher = sql_get_var_row("select voucherid,nama,ringkas,alias,published,id,term,harga,startdate,enddate,startusedate,endusedate,qty,views,gambar,id_wolrd_sec from tbl_world_voucher where voucherid='$voucherid'");

	$idkategori = $voucher['id_wolrd_sec'];
	$kategori = sql_get_var("SELECT namasec FROM tbl_world_sec WHERE secid='$idkategori' ");
	...
	if (isset($_POST['bayar'])) {
		...
		$perintah = ("INSERT INTO tbl_transaksi_all(invoice,userid,nama_produk,harga,fee,jumlah,payment_type,tanggal,datetransfer,tanggal_pembayaran,status_transaksi) VALUES ('$orderid', '$userid', '$nama', '$harga', 4000, '$harga2', '', '$date', '$datetransfer', '', 'INCART' ) ");
		$prt = sql($perintah);

		$query	= "insert into tbl_transaksi_world(`transaksiid`,worlduserid,voucherid,`invoiceid`,`status`,userid,jml,create_date, create_userid,update_userid,adminfee,totaltagihan,totalinvoice,batastransfer,simposium,kategori_simpo) 
						values ('$new','$wuserid','$voucherid','$orderid','0','$userid','$jml','$date','0','0','4000','$harga2','$harga2','$datetransfer','1','$kategori')";
		$hasil = sql($query);

		if ($hsl) {

			$subject			= "Simposium Nasional, Tagihan eVoucher #$orderid";
			...
			$item_details[] = array('id' => "2", 'price' => 4000, 'quantity' => 1, 'name' => "Fee Payment Gateway");
...
$tpl->display("payment-simposium.html");
exit();
