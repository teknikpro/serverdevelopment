...
if(!empty($katid))
{
	$perintah1 = "select * from tbl_khazanah_sec where alias='$katid'";
...
$sql = "select count(*) as jml from tbl_konten where published='1' and tipe='text' $where";
$hsl = sql( $sql);
...
$mysql = "select id,ringkas,nama,create_date,alias,secid,gambar,gambar1 from tbl_konten where published='1'  and tipe='text' $where order by id  desc limit $ord, $judul_per_hlm";
$hasil = sql( $mysql);
...
while ($data =  sql_fetch_data($hasil)) {	
	...
	if(!empty($gambar)) $gambar = "$domain/gambar/text/$gambar";
	 else $gambar = "";
		 

	$link = "$fulldomain/konten/read/$secalias1/$id/$alias.html";
...
if ($hlm > 1){
	$prev = $hlm - 1;
	$stringpage[$pageid] = array("nama"=>"Awal","link"=>"$domainfull/$username/$aksi/1");
	$pageid++;
	$stringpage[$pageid] = array("nama"=>"Sebelumnya","link"=>"$domainfull/$username/$aksi/$prev");
...
for ($ii=$hlm3; $ii <= $hlm_tot and $ii<=$hlm4; $ii++){
	if ($ii==$hlm){
		...
		$stringpage[$pageid] = array("nama"=>"$ii","link"=>"$domainfull/$username/$aksi/$ii");
	}
	$pageid++;
}
if ($hlm < $hlm_tot){
	$next = $hlm + 1;
	$stringpage[$pageid] = array("nama"=>"Selanjutnya","link"=>"$domainfull/$username/$aksi/$next");
	$pageid++;
	$stringpage[$pageid] = array("nama"=>"Akhir","link"=>"$domainfull/$username/$aksi/$hlm_tot");
}
else
{
