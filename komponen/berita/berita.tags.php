...
$mysql = "select id,ringkas,nama,create_date,alias,secid,gambar,gambar1 from tbl_$kanal where published='1' $where order by create_date  desc limit $ord, $judul_per_hlm";
$hasil = sql( $mysql);
...
if(!empty($gambar)) $gambar = "$domain/gambar/$kanal/$gambar";
	...
	$datadetail[$id] = array("id"=>$id,"no"=>$i,"namasec"=>$namasec,"urlsec"=>$urlsec,"nama"=>$nama,"ringkas"=>$ringkas,"tanggal"=>$tanggal1,"date"=>$tanggal,"alias"=>$alias,"link"=>$link,"gambar"=>$gambar);
	$i++;
