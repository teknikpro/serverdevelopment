...
switch ($subaksi)
{
	case 'add':
		if($_POST['save']=="save")
		{
			...
			$dirname = "$pathfile/voucher/$produkpostid";
			...
			if($hasil)
			{
				...
				$style		= "alert-success";
				$backlink	= "$fullmanage/mitra/voucher/detail/$produkpostid/";
			}
			else
			{
	...
	case 'edit':
		...
		else
		{
			...
			$sql = sql($sql);
			while ($row = sql_fetch_data($sql))
				$gambar[$row['albumid']] = array('albumid'=>$row['albumid'], 'gambar'=>"$fulldomain/gambar/voucher/$produkpostid/$row[gambar_s]");
			sql_free_result($sql);
	...
	case 'hapusgambar':
		...
		unlink("$pathfile/voucher/$produkpostid/$gambar_s");
		unlink("$pathfile/voucher/$produkpostid/$gambar_m");
		unlink("$pathfile/voucher/$produkpostid/$gambar_l");
		unlink("$pathfile/voucher/$produkpostid/$gambar_f");

		$query = sql("DELETE FROM tbl_product_image WHERE albumid='$albumid'  and userid='$_SESSION[userid]' ");

		if ($query)
		{
			header("location: $fulldomain/mitra/voucher/detail/$produkpostid/");
			exit();
		}
	...
	case 'publish':
		...
		if ($query)
			header("Location: $fulldomain/mitra/voucher/");
	...
	case 'detail':
		...
		$sql = sql($sql);
		while ($row = sql_fetch_data($sql))
			$gambar[$row['albumid']] = array('albumid'=>$row['albumid'], 'gambar'=>"$fulldomain/gambar/voucher/$produk[produkpostid]/$row[gambar_s]");
		sql_free_result($sql);
	...
	default:
		...
		$sql = "select count(*) as jml from tbl_world_voucher where id='$pageid'";
		$hsl = sql($sql);
		...
		$perintah	= "select voucherid,nama,ringkas,published,id,term,harga,startdate,enddate,startusedate,endusedate,qty,views,gambar from tbl_world_voucher where id='$pageid' order by voucherid desc limit $ord, $judul_per_hlm";
		$hasil		= sql($perintah);
		$datadetail	= array();
		$no = 0;
		while($data = sql_fetch_data($hasil))
		{
			...
			if($status=="1") $status = '<span class="label label-success">Aktif</span>';
			else $status = '<span class="label label-default">Tidak Aktif</span>';

			
			if(!empty($gambar)) $gambar = "$domain/gambar/world/$gambar";
			 else $gambar = "$fulldomain/images/noimages.jpg";
			 
			$link = "$fulldomain/$kanal/voucher/detail/$id";
			 
			$no++;
			$datadetail[] = array("id"=>$id,"no"=>$i,"nama"=>$nama,"ringkas"=>$ringkas,"harga"=>$harga,"status"=>$status,"published"=>$published,"published1"=>$data['published'],"status_tayang"=>$data['status_tayang'],"publish"=>$publish,"tanggal"=>$tanggal,"alias"=>$alias,"link"=>$link,"gambar"=>$gambar);
		}
