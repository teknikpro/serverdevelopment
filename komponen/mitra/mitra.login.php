...
if(isset($_POST['user']))
{
	...
	$pass 		= $_POST['pass'];
	$password 	= md5($pass);	
	$perintah	= "select userid,userfullname,username,userdirname,avatar,verified,fbcid,twcid,gpcid,tipe,userlastloggedin from tbl_member where (username='$user' or useremail='$user') and userpassword='$password' and useractivestatus='1' and tipe='3'";
	$hasil 		= sql($perintah);
	
	if(sql_num_rows($hasil)<1)
	{
		header("location: $fulldomain/mitra/usernameerror");
		exit();
	}
	else
	{
		...
		if((!$_SESSION['last']) || ($_SESSION['last']=="/")) $pelempar = "$fulldomain/mitra"; 
