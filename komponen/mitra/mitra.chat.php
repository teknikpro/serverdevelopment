...
if($subaksi=="stream")
{
	...
	switch($function) {
		 ...
		 case('update'):
			...
			if($lastid==0)
			{
				$sql = "select id,pesan,create_date,status,to_userid,from_userid from tbl_chat_message where chat_id='$chatid'";
			}
			else
			{
				$sql = "select id,pesan,create_date,status,to_userid,from_userid from tbl_chat_message where chat_id='$chatid' and id > $lastid";
			}
			...
			while($dt=sql_fetch_data($hsl))
			{
				...
				{
					$text[] = array("id"=>$id,"pesan"=>"<li class=\"message-right animated fadeinleft\"><img alt=\"\" src=\"$gambar\"><div class=\"message first\"><p>$pesan</p></div><span>$tgltampil</span>");
				}
				else
				{
					$text[] = array("id"=>$id,"pesan"=>"<li class=\"message-left animated fadeinright\"><img alt=\"\" src=\"$gambar\"><div class=\"message first\"><p>$pesan</p></div><span>$tgltampil</span>");
				}
			}
			$log['text'] = $text; 
...
{		
		...
		$perintah	= "select from_userid,chat_id,to_userid,create_date from tbl_chat where to_userid='$_SESSION[userid]' order by create_date desc limit $ord, $judul_per_hlm";
		$hasil		= sql($perintah);
		$datadetail	= array();
		$no = 0;
