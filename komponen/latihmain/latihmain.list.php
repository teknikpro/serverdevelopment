...
$sql = "select count(*) as jml from tbl_video where published='1' $where";
$hsl = sql( $sql);
...
$mysql = "select id,ringkas,nama,create_date,alias,gambar,gambar1,jenis,youtubeid from tbl_video where published='1' $where order by id  desc limit $ord, $judul_per_hlm";
$hasil = sql( $mysql);
...
while ($data =  sql_fetch_data($hasil)) {	
	...
	$datadetail[$id] = array("id"=>$id,"no"=>$i,"nama"=>$nama,"namasec"=>$namasec,"ringkas"=>$ringkas,"tanggal"=>$tanggal,"alias"=>$alias,"link"=>$link,"gambar"=>$gambar);
	$i++;
