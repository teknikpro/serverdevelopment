...
else
{
	...
	if(isset($_POST['paketid']))
	{
		...
		$paket = sql_get_var_row("select nama,harga from tbl_member_sec where secid='$paketid'");
		$harga = $paket['harga'];
		...
		$new = newid("transaksiid","tbl_transaksi_upgrade");
			
		$query	= "insert into tbl_transaksi_upgrade(`transaksiid`,paketid,`invoiceid`,`status`,userid,create_date, create_userid,update_userid,totaltagihan,totalinvoice,batastransfer) 
		...
		if($hsl)
		{
			...
			$subject			= "$title, Tagihan Upgrade #$orderid";
			...
			$item_details = array();
			$item_details[] = array('id' => "1",'price' => $harga,'quantity' => 1,'name' => "Upragde $nama - $userfullname");
			...
			$transaction = array(
				"vtweb" => array (
				  ...
				  "finish_redirect_url" => "$fulldomain/bayar/upgrade/finish/sukses/",
				  "unfinish_redirect_url" => "$fulldomain/bayar/upgrade/finish/gagal/",
				  "error_redirect_url" => "$fulldomain/bayar/upgrade/finish/error/"
				),
	...
	{
		$namapaket = sql_get_var("select nama from tbl_member_sec where secid='$paket'");
		$message = "Mohon maaf saat ini telah terdaftar dipaket yang anda pilih yaitu $namapaket. Transaksi anda tidak akan kami lanjutkan. Terima kasih";
		$tpl->assign("message",$message);
	}
