...
if ($benar) {
	...
	if ($_SESSION['username'] or ($jenis == 1)) {
		...
		if ($jenis == 1) {
			...
			$urlconfirm = "$fulldomain/bayar/transaction/$_SESSION[invoice]";
		} else {
			...
			$urlconfirm = "$fulldomain/bayar/transaction";
		}
		...
		while ($data = sql_fetch_data($hasil)) {
			...
			$bank[$id] = array("id" => $id, "nama" => $nama, "bankid" => $id_bank);
		}
		sql_free_result($hasil);
		...
		header("location: $fulldomain/user/bayar");
	}
} else {
