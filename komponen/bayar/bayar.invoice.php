...
if ($_SESSION['orderid']) {
	...
	if ($_SESSION['username'] or ($jenis == 1)) {
		...
		$ongkoskirim		= $_POST['ongkoskirim']; //print_r($ongkoskirim);die();
		$totalongkoskirim 	= $_POST['totalongkoskirim'];
		$listresellerid = implode(",", $resellerid);
		...
		if (sql_num_rows($hasil) > 0) {
			...
			$perintah 	= "update tbl_transaksi set invoiceid='$invoiceid', namalengkap='$userfullname', email='$email', alamatpengiriman='$alamat_kirim', propid='$propinsiid',kotaid='$kotaid',kecid='$kecid',
						 ongkoskirim='$totalongkoskirim', pengiriman='$pengiriman',agen='$pengiriman', pembayaran='$pembayaran', tanggaltransaksi='$tanggal', batastransfer='$datetransfer', pesan='$pesan', dropship='$dropship', namapengirim='$namapengirim', telppengirim='$telppengirim', status='1', userid='$_SESSION[userid]', resellerid = '$listresellerid'  
			...
			if (!$hasil) {
				...
				$ongkirs = 0;
				foreach ($resellerid as $valuesellerid) {
					//die($valuesellerid);
					$ordertokoid = "FS-TK$valuesellerid" . date("U");
					sql("update tbl_transaksi_detail set ordertokoid='$ordertokoid' where transaksiid='$transaksiid' and resellerid='$valuesellerid'");

					$ongkirid = $ongkoskirim["$valuesellerid"];
					...
					$new = newid("id", "tbl_transaksi_ongkir");

					sql("insert into tbl_transaksi_ongkir (`id`, `create_date`, `create_userid`, `update_userid`, `transaksiid`, `resellerid`, `agen`, `ongkosinfo`, `ongkoskirim`) value ('$new','$tanggal','0','0','$transaksiid','$valuesellerid','$agen', '$ongkosinfo', '$ongkoskirimm')");
					$ongkirs = $ongkirs + $ongkoskirimm;
				}
				...
				if ($pembayaran == "Transfer") {
					...
					while ($row = sql_fetch_data($hsl)) {
						...
						$newids = newid("produkstokonholdid", "tbl_product_stokonhold");

						$perintahst 	= "insert into tbl_product_stokonhold (produkstokonholdid,`invoiceid`, `produkpostid`, `jumlah`) values ('$newids','$invoiceid', '$produkpostid', '$qty')";
					...
					$tpl->assign("detailtoko", $dt_toko);
					$tpl->assign("detailproduk", $dt_keranjang);

					//Update Metode Pembayaran
					$update = sql("update tbl_transaksi set totalinvoice='$totalinvoice' where orderid='$_SESSION[orderid]'");


					$ongkoskirim2	= "$matauang. " . number_format($totalongkoskirim, 0, ".", ".");
					...
					if ($totaltagihanafterdiskon == 0)
						$totaltagihanakhir = $totaltagihan + $totalongkoskirim;
					else
						$totaltagihanakhir = $totaltagihanafterdiskon + $totalongkoskirim;
					...
					$tpl->assign("ongkos_kirim", $totalongkoskirim);
					...
					$sendmail	= sendmail($title, $to, $subject, $html, $html);
					...
					unset($_SESSION['orderid'], $_SESSION['last'], $_SESSION['jenis']);
				} else if ($pembayaran == "CreditCard" || $pembayaran == "ATMTransfer" || $pembayaran == "MandiriClickPay" || $pembayaran == "MandiriBill" || $pembayaran == "EpayBRI" || $pembayaran == "GoPay") {
					...
					if ($ongkoskirim != "0") {
						$item_details[] = array('id' => $a, 'price' => $ongkirs, 'quantity' => "1", 'name' => "Ongkos Kirim");
					}
					...
					else if ($pembayaran == "GoPay") $metode = "gopay";

					// Fill transaction details
					$transaction = array(
						"vtweb" => array(
							"enabled_payments" => array("$metode"),
							"finish_redirect_url" => "$fulldomain/bayar/finish/sukses/",
							"unfinish_redirect_url" => "$fulldomain/bayar/finish/gagal/",
							"error_redirect_url" => "$fulldomain/bayar/finish/error/"
						),
		...
		$last = "$fulldomain/bayar/checkout";
		$_SESSION['last']	= $last;
