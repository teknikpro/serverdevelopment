...
$sqlp	= "select produkpostid,secid,subid,brandid,title,ringkas,harga,icon,numviews,vidio from tbl_product_post where status='1' and published='1' and simposium='1' and buat='2' $where order by $sorting desc limit $ord, $judul_per_hlm";
$queryp	= sql($sqlp);
...
while ($row2 = sql_fetch_data($queryp)) {
	...
	$link_detail	= "$fulldomain/lelang/read/$produkpostid/$alias?menu=lelang/read&id=$produkpostid";
