...
$sqlp	= "select produkpostid,secid,subid,brandid,title,ringkas,harga,icon,numviews,vidio from tbl_product_post where status='1' and published='1' and simposium='1' and buat='1' $where order by $sorting desc limit $ord, $judul_per_hlm";
$queryp	= sql($sqlp);
...
while ($row2 = sql_fetch_data($queryp)) {
	...
	$link_detail	= "$fulldomain/charity/read/$produkpostid/$alias?menu=charity/read&id=$produkpostid";
	...
	$list_post[$produkpostid]	= array(
		...
		"misc_harga_reseller" => $misc_harga_reseller, "hargares" => $misc_hargares2, "wishlist" => $wishlist, "icon" => $icons, "numviews" => $numviews, "jumlahlike" => $jumlahlike, "statuslike" => $ceklikeid, "vidio" => $vidio
	);
	$no++;
}
