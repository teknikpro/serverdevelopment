...
$sql	= "select kodeproduk, title, content, ringkas, harga,stock,cart, body_dimension, body_weight,  create_date, userid,ispinjam,ispesan, brandid,icon,numviews,vidio,pembuat from tbl_product_post where status='1' and produkpostid='$produkpostid' and published='1'";
$query	= sql($sql);
...
$cekpekarya = "SELECT id_perkarya,foto,nama,penjelasan FROM tbl_pekarya WHERE produkid='$produkpostid' ";
...
// Gambar
$ambilgambar = sql_get_var("SELECT gambar_f FROM tbl_product_image WHERE produkpostid='$produkpostid' AND utama='1' ");
...
if ($_POST['aksi'] == "simpan") {
	...
	if ($benar == 2) {
		...
		if ($hasil)
			header("location: $fulldomain/lelang/read/$produkpostid/$detailalias");
	}
}
