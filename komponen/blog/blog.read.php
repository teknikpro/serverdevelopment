...
if (!empty($katid) && $katid != "global") {
	...
	$tpl->assign("arsip", "$fulldomain/blog/list/$secalias/");
} else {
	$secalias = "global";
	$tpl->assign("rubrik", "$rubrik");
	$tpl->assign("arsip", "$fulldomain/blog/list/$secalias/");
...
$perintah = "select id,nama,oleh,ringkas,lengkap,gambar1,gambar,create_date,alias,views,tags,infogambar,foto_penulis,penyunting from tbl_$kanal where published='1' and id='$id'";
$hasil = sql($perintah);
...
$foto_penulis = $data['foto_penulis'];
$penyunting = $data['penyunting'];
...
$tpl->assign("detailsumber", $infogambar);
$tpl->assign("penyunting", $penyunting);
...
if (!empty($foto_penulis)) $tpl->assign("foto_penulis", "$fulldomain/gambar/$kanal/$foto_penulis");


//SetAction
if ($_SESSION['userid'] && $_SESSION['usertipe'] != "0") {
	setaction($_SESSION['userid'], "membaca artikel <b>$nama</b>", "/blog/read/$secalias/$idcontent/$alias");
}
...
//Berita Pilihan
$mysql = "select id,ringkas,nama,create_date,alias,gambar,secid,infogambar,foto_penulis from tbl_$kanal where published='1' and id!='$id' order by rand() limit 4";
$hasil = sql($mysql);
...
while ($data = sql_fetch_data($hasil)) {
	...
	if (!empty($foto_penulis)) $foto_penulis = "$fulldomain/gambar/$kanal/$foto_penulis";
	else $foto_penulis = "";
	$link = "$fulldomain/$kanal/read/$secalias/$id/$alias";

	$datapilihan[$id] = array("id" => $id, "a" => $a, "nama" => $nama, "ringkas" => $ringkas, "namasec" => $namasec, "tanggal" => $tanggal, "alias" => $alias, "link" => $link, "gambar" => $gambar, "infogambar" => $infogambar, "$foto_penulis" => "foto_penulis");
	$a++;
}
$a = 0;
sql_free_result($hasil);
...
//Komentar Blog
$mysql2 = "SELECT id_blog_komentar,id_blog,userid,komentar FROM tbl_blog_komentar WHERE id_blog='$idblog' ";
$hasil2 = sql($mysql2);

$komentarblog = array();
$a = 1;
while ($data2 = sql_fetch_data($hasil2)) {
	$id_blog_komentar		= $data2['id_blog_komentar'];
	$id_blog				= $data2['id_blog'];
	$komentar				= $data2['komentar'];
	$userid					= $data2['userid'];

	$photo = sql_get_var("SELECT avatar FROM tbl_member WHERE userid='$userid' ");
	$nama = sql_get_var("SELECT userfullname FROM tbl_member WHERE userid='$userid' ");

	if($photo){
		$photo = "https://www.serverdevelopment2020.my.id/uploads/avatars/" . $photo;
	}else {
		$photo = "https://www.serverdevelopment2020.my.id/template/dfunstation/newtemplate/assets/img/icon-user.png";
	}

	$komentarblog[] = array("id_blog_komentar" => $id_blog_komentar, "id_blog" => $id_blog, "komentar" => $komentar, "photo" => $photo, "nama" => $nama);
	$a++;
}
$a = 0;
sql_free_result($komentarblog);
$tpl->assign("login", $_SESSION['userid']);
$tpl->assign("komentar", $komentarblog);
