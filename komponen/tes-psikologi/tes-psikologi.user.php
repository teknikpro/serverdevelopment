...
{
	header("location: $fulldomain/termometer-kecemasan/login");
	exit();
}
if(isset($_POST['update']))
{
	...
	$query= "update tbl_tes_user set userfullname='$userfullname',usergender='$usergender',kecid='$kecid',kotaid='$kotaid',marriagestatusid='$marriagestatusid',pekerjaan='$pekerjaan',
	...
	if($hasil)
	{
		header("location: $fulldomain/termometer-kecemasan/tes");
		exit();
	}
...
else
{
	if($_SESSION['userid'])
	{
		$perintah = sql("select * from tbl_tes_user where userid='$_SESSION[userid]'");
