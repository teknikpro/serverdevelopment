...
$sqlp	= "select produkpostid,secid,subid,brandid,title,ringkas,harga,icon from tbl_product_post where status='1' and published='1' $where order by $sorting desc limit $ord, $judul_per_hlm";
$queryp	= sql($sqlp);
...
while($row2 = sql_fetch_data($queryp))
{
	...
	$list_post[$produkpostid]	= array("produkpostid"=>$produkpostid,"namaprod"=>$namaprod,"image_m"=>$image_m,"link_detail"=>$link_detail,"ringkas"=>$ringkas,
