...
$mysql = "select userid,username,userfullname,avatar,konsulsecid,konsulsubid,hargakonsultasi,online,rating,aboutme,nama_depan,nama_belakang,gelar,keahlian,kutipan,no_izin,gambardum from tbl_member where tipe='1' and peer='0' and guru='0' and status_konsultan='0' and tampil_depan='1'  order by rating asc limit 8";
$hasil = sql($mysql);
...
while ($data =  sql_fetch_data($hasil)) {
	...
	$url = "/$kanal/profile/$encryptedData";

	$datadepankonsultan[] = array("id" => $idx, "no" => $i, "nama" => $nama, "sec" => $sec, "url" => $url, "ringkas" => $ringkas, "harga" => $harga, "hargarp" => $hargarp, "online" => $online, "avatar" => $avatar, "rating" => $rating, "nama_depan" => $nama_depan, "nama_belakang" => $nama_belakang, "gelar" => $gelar, "keahlian" => $keahlian, "kutipan" => $kutipan, "no_izin" => $no_izin, "gambardum" => $gambardum );
	$i++;
}
sql_free_result($hasil);
