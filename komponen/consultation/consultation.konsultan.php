...
$mysql = "select userid,username,userfullname,avatar,konsulsecid,konsulsubid,hargakonsultasi,online,rating,aboutme from tbl_member where tipe='1' and peer='0' and guru='0' and status_konsultan='0' $where order by rating desc limit $ord, $judul_per_hlm";
$hasil = sql($mysql);
...
while ($data =  sql_fetch_data($hasil)) {
	...
	$datadetail[] = array("id" => $id, "no" => $i, "nama" => $nama, "sec" => $sec, "harga" => $harga, "hargarp" => $hargarp, "url" => $url, "online" => $online, "avatar" => $avatar, "rating" => $rating, "ringkas" => $ringkas, "katid" => $katid);
	$i++;
}
