...
while($data = sql_fetch_data($hasil))
{
	...
	{ 
		...
		$no++;
		$datadetail[$id] = array("id"=>$id,"no"=>$i,"user"=>$user,"tanggal"=>$tanggal,"link"=>$link,"gambar"=>$gambar,"status"=>$publish,"jmlchat"=>$jml,"jmlnoread"=>$jmlbelum,"namamerchant"=>"merchant");
	}
}
...
$namaMerchant = sql_get_var("SELECT nama FROM tbl_world WHERE id='$idMerchant'");

$tpl->assign("datadetail",$datadetail);
$tpl->assign("namamerchant",$namaMerchant);
