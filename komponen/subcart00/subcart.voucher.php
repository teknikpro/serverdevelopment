...
else
{
	...
	if(isset($_POST['chatid']))
	{
		...
		$harga = $konsultan['hargakonsultasi'];
		$harga1 = rupiah($harga);
		$nama = "Voucher Konsultasi $konsultan[nama]";
		...
		$orderid = "VKDFS".date("U");
		
		$datetransfer = date("Y-m-d H:i:s", strtotime("+$jedatransfer hour"));
		$new = newid("transaksiid","tbl_transaksi_voucher");
			
		$query	= "insert into tbl_transaksi_voucher(`transaksiid`,chat_id,`invoiceid`,`status`,userid,create_date, create_userid,update_userid,adminfee,totaltagihan,totalinvoice,batastransfer) 
						values ('$new','$chatid','$orderid','0','$userid','$date','0','0','4000','$harga2','$harga','$datetransfer')";
		$hasil = sql($query);
